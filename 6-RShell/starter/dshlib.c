        ...
        if (strcmp(cmd->argv[0], "cd") == 0) {
            if (cmd->argc == 1) {
                ...
                if (chdir(cmd->argv[1]) != 0) {
                    perror("cd error");
                    last_rc = 1; // Set error code on failure
                } else {
                    last_rc = 0; // Success
                }
                return 1;
                ...
                last_rc = 1; // Set error code for invalid syntax
                return 1;
            }
        }
...
int exec_local_cmd_loop() {
    ...
    // Don't print the initial prompt before executing commands
    // printf("%s", SH_PROMPT);
    // fflush(stdout);
    ...
    while (1) {
        // Don't print the prompt in the loop for non-interactive mode
        if (interactive) {
            printf("%s", SH_PROMPT);
            fflush(stdout);
        }
